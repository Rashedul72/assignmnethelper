...
function prepStops(stops?: string[]): { arr: RGB[]; count: number } {
...
export default function GradientBlinds({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      callIfFn(programRef.current, "remove" as keyof Program);
      callIfFn(geometryRef.current, "remove" as keyof Triangle);
      callIfFn(meshRef.current, "remove" as keyof Mesh);
      // ogl Renderer doesn't expose a `destroy` method on all versions —
      // we just drop the reference and let GC + WEBGL_lose_context handle it.
