...
// face-api.js types only — the runtime module is loaded dynamically inside
// the webcam effect, so it is *not* part of the initial JS bundle when
// enableWebcam is false (the default for our hero).
...
function smoothDampVec2(
  ...
  const movedTarget = current.clone().sub(change);
  ...
  out.copy(movedTarget.clone().add(change.add(temp).multiplyScalar(exp)));
...
function smoothDampFloat(
  ...
  let out = movedTarget + (change + temp) * exp;
...
export const GridScan = ({
  ...
  // Pointer / click input (works with or without webcam).
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        // iOS-only static permission API; use a runtime check.
        typeof (
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: uniforms as unknown as Record<
        string,
  ...
  // Live uniform updates when props change without rebuilding the renderer.
  useEffect(() => {
    const m = materialRef.current;
    if (m) {
      const u = m.uniforms;
  ...
  // Face-api model loader. Only runs when webcam tracking is requested,
  // so we do not pull ~10 MB of weight files for the default hero usage.
  useEffect(() => {
    if (!enableWebcam) {
  ...
  // Webcam capture + face detection loop.
  useEffect(() => {
  ...
  return (
    ...
    >
      {showPreview && (
        <div className="absolute right-3 bottom-3 w-[220px] h-[132px] rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-black text-white text-xs leading-tight pointer-events-none">
          <video
            ref={videoRef}
            muted
            playsInline
          ...
          />
          <div className="absolute left-2 top-2 px-1.5 py-0.5 bg-black/50 rounded backdrop-blur-sm">
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? "Face: tracking"
