...
/**
 * Hyperspeed — TypeScript port of the React Bits component.
 *
 * Notes / changes vs. the original JS:
 *  - Strongly typed public API (props + colour palette).
 *  - The internal three.js / postprocessing integration is large and uses
 *    raw shader chunks; rather than typing every internal helper class
 *    (Road, CarLights, LightsSticks, App) we keep them as locally scoped
 *    `any` to avoid noise. Only the surface area is typed.
 *  - Defensive cleanup so React Strict Mode + Fast Refresh don't leak
 *    WebGL contexts.
 */
...
const DEFAULT_EFFECT_OPTIONS: Required<
  Omit<HyperspeedEffectOptions, "onSpeedUp" | "onSlowDown" | "colors">
> & {
  onSpeedUp: (ev: Event) => void;
  onSlowDown: (ev: Event) => void;
...
export default function Hyperspeed({
  ...
  const hyperspeedRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    const distortions: Record<Distortion, any> = {
      mountainDistortion: {
    ...
    class CarLights {
      ...
      init() {
        ...
        (instanced as any).instanceCount = options.lightPairsPerRoadWay * 2;
      ...
      update(time: number) {
        ((this.mesh.material as THREE.ShaderMaterial).uniforms as any).uTime.value =
          time;
      }
    }
    ...
    class LightsSticks {
      ...
      init() {
        ...
        (instanced as any).instanceCount = totalSticks;
      ...
      initPasses() {
        ...
        const smaaPass = new EffectPass(
          ...
          new SMAAEffect({
            ...
            searchImage: SMAAEffect.searchImageDataURL as any,
            areaImage: SMAAEffect.areaImageDataURL as any,
          } as any)
        );
      ...
      loadAssets() {
        ...
        return new Promise<void>((resolve) => {
          ...
          searchImage.addEventListener("load", function () {
            ...
            (manager as any).itemEnd("smaa-search");
          });

          areaImage.addEventListener("load", function () {
            assets.smaa.area = this;
            (manager as any).itemEnd("smaa-area");
          });
          (manager as any).itemStart("smaa-search");
          (manager as any).itemStart("smaa-area");

          searchImage.src = SMAAEffect.searchImageDataURL as any;
          areaImage.src = SMAAEffect.areaImageDataURL as any;
        });
      }

      init() {
        // Guard against React Strict Mode (and Fast Refresh): the effect
        // can dispose this App while loadAssets() is still in flight. When
        // the SMAA image promises resolve, .then(init) would run against
        // a renderer whose WebGL context has been forcibly lost, making
        // postprocessing's addPass() crash on `getContextAttributes().alpha`.
    ...
    options.distortion = distortions[options.distortion as Distortion];

    const myApp = new App(container, options);
    appRef.current = myApp;
    void myApp.loadAssets().then(myApp.init);
